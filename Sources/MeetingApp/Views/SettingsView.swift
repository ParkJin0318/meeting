...
struct SettingsView: View {
    ...
    private var vaultHint: String {
        ...
        var missing = ["raw/", "wiki/journal/log.md"].filter {
            !fm.fileExists(atPath: root.appendingPathComponent($0).path)
        }
        // 벤더링본은 script/ 와 루트 둘 다 인정한다
        if WikimapRefresher.locateScript(in: root, fileManager: fm) == nil {
            missing.append("wikimap.py")
        }
