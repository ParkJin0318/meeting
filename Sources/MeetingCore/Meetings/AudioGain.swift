...
public enum AudioGain {
    ...
    // 말소리/바닥소음 비. 정상 소음은 1.1 언저리(백색잡음 1.06)이고 말이 섞이면 2 를 넘는다.
    // 3.0 이면 강의처럼 쉬는 틈 없이 말하는 녹음이 막힌다 — 말이 촘촘할수록 p10(바닥)이 말소리로
    // 채워져 비가 오히려 작아진다(실측 3시간 강의: 최소 1.87 · 중앙 2.81). 게이트가 거꾸로 걸려
    // 30초 창의 62%가 증폭 없이 나갔고, 그 크기(프레임 RMS ~0.02)가 whisper VAD 기본 임계와
    // 겹쳐 전사의 94%가 사라졌다.
    static let speechDynamicRange: Float = 1.6
