...
public struct MeetingVaultExporter: Sendable {
    ...
    var notesRoot: URL {
        vaultRoot.appendingPathComponent("wiki/journal/meetings")
    }

    var logURL: URL {
        vaultRoot.appendingPathComponent("wiki/journal/log.md")
    }
    ...
    public func export(_ meeting: Meeting) throws -> Written? {
        ...
        do {
            ...
            warnings.append("wiki/journal/log.md 기록 실패 — \(error.localizedDescription)")
        }
