...
@Suite struct MeetingVaultExportTests {
    ...
    @Test func writesTranscriptToRawAndSummaryToNotes() throws {
        ...
        #expect(written.notePath.hasPrefix(root.appendingPathComponent("wiki/journal/meetings").path))
    ...
    @Test func reexportOverwritesInsteadOfDuplicating() throws {
        ...
        let notes = try FileManager.default.contentsOfDirectory(
            atPath: root.appendingPathComponent("wiki/journal/meetings").path)
    ...
    @Test func renameMovesNoteToNewNameAndFixesLinks() async throws {
        ...
        let linking = root.appendingPathComponent("wiki/journal/기획.md")
...
@Suite struct AudioGainTests {
    ...
    @Test func continuousSpeechOverAHighFloorIsStillAmplified() {
        // 강의 실측 모양 — 세 시간 내내 말이 이어져 바닥(p10)이 말소리로 차 있고
        // 말소리/바닥 비가 2 언저리다. 여기서 증폭이 막히면 whisper VAD 가 통째로 건너뛴다.
        var samples = speech(level: 0.02, noise: 0.009, seconds: 90)
        #expect(AudioGain.hasSpeech(AudioGain.measure(samples)), "이어지는 말을 잡음으로 봤다")
        AudioGain.normalizeWindowed(&samples)
        #expect(AudioGain.measure(samples).speech > 0.05, "목표(0.08) 근처까지 올라오지 않았다")
        #expect(samples.allSatisfy { $0 >= -1 && $0 <= 1 })
    }

    @Test func steadyNoiseWithoutSpeechIsStillRejected() {
        var generator = SystemRandomNumberGenerator()
        let hiss = (0..<(16_000 * 4)).map { _ in
            Float.random(in: -0.02...0.02, using: &generator)
        }
        #expect(!AudioGain.hasSpeech(AudioGain.measure(hiss)), "고른 잡음을 말로 봤다")
    }
