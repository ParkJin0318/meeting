...
@Suite struct VaultConformanceTests {
    private func makeVault(withLog: Bool = true) throws -> URL {
        ...
        try FileManager.default.createDirectory(at: root.appendingPathComponent("wiki/journal/meetings"),
                                                withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: root.appendingPathComponent("raw"),
                                                withIntermediateDirectories: true)
        if withLog {
            try "# 위키 로그\n\n## [2026-01-01] Init: 시작\n".write(
                to: root.appendingPathComponent("wiki/journal/log.md"), atomically: true, encoding: .utf8)
        }
        return root
    }
    ...
    @Test func writesNFCNamesAndLinks() throws {
        ...
        let noteEntries = try diskNames(root.appendingPathComponent("wiki/journal/meetings"))
    ...
    @Test func repairsNFDEntriesOnReexport() throws {
        ...
        var meeting = sample()
        let expectedName = "2026-08-26-검색팀-자동완성-순위-개편-Kick-off-미팅.md"
        let nfdNote = root.appendingPathComponent("wiki/journal/meetings")
        ...
        try #require(try !isOnDisk(expectedName, in: root.appendingPathComponent("wiki/journal/meetings")))
        ...
        let notes = root.appendingPathComponent("wiki/journal/meetings")
        ...
        let log = try String(contentsOf: root.appendingPathComponent("wiki/journal/log.md"), encoding: .utf8)
    ...
    @Test func appendsIngestLineOnceOnFirstExport() throws {
        ...
        let expected = "## [2026-09-02] Ingest: 미팅 노트 — 검색팀 - 자동완성 순위 개편 Kick off 미팅 (2026-08-26, 49분) → "
            + "wiki/journal/meetings/2026-08-26-검색팀-자동완성-순위-개편-Kick-off-미팅.md · "
        ...
        let again = try String(contentsOf: root.appendingPathComponent("wiki/journal/log.md"), encoding: .utf8)
    ...
    @Test func renameWritesFixLineAndRewritesLegacyLinks() throws {
        ...
        let referrer = root.appendingPathComponent("wiki/journal/other.md")
        ...
        #expect(try isOnDisk("2026-08-26-자동완성-킥오프.md", in: root.appendingPathComponent("wiki/journal/meetings")),
                "옮긴 엔트리가 NFD다")
    ...
    @Test func skipsLogWhenVaultHasNone() throws {
        ...
        let written = try #require(try exporter(root).export(sample()))
        #expect(written.warnings.isEmpty)
        #expect(!FileManager.default.fileExists(atPath: root.appendingPathComponent("wiki/journal/log.md").path))
    }
    ...
    @Test func wikimapRefresherSkipsWithoutScriptAndCallsUpdate() async throws {
        ...
        // 지금 배치: script/wikimap.py 가 있으면 루트본보다 먼저 잡힌다
        let scriptDir = root.appendingPathComponent("script")
        try FileManager.default.createDirectory(at: scriptDir, withIntermediateDirectories: true)
        let nested = scriptDir.appendingPathComponent("wikimap.py")
        try "print('ok')\n".write(to: nested, atomically: true, encoding: .utf8)
        #expect(await refresher.refresh(vaultRoot: root) == .ran)
        #expect(try #require(runner.calls.last).arguments.first == nested.path)
