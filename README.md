...
기본은 꺼져 있다. 설정에서 경로를 넣으면 그 아래에 마크다운을 쓴다 — 요약 노트는 `wiki/journal/meetings/`, 전사 원문은 `raw/`, 변경 이력은 `wiki/journal/log.md`. 경로 안에 `wiki/` 디렉토리가 있어야 vault로 인정한다.

vault 에 `script/wikimap.py`(옛 배치는 루트의 `wikimap.py`)가 있으면 저장 후 실행해 색인을 갱신한다. 없으면 그냥 건너뛴다.
